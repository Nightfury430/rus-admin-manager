            ...
            fill_prices: function(){
                if(this.item.params.materials_mode == 'i'){
                    ...
                    Object.keys(selected).forEach((k)=>{
                        let item = copy_object(def_item)
                        item.type = '2';
                        item.value = selected[k].id
                ...
                if(this.item.params.materials_mode == 'c'){
                    ...
                    for (let i = 0; i < selected.length; i++) {
                        ...
                        let item = copy_object(def_item)
                        item.type = '0';
                        item.value = selected[i].id;
                        this.item.items.push(item)
                    }
                }
            },

            change_item_price_type(ind){
                console.log(this.item.items[ind].type)
                switch (this.item.items[ind].type) {
                    case('1'):
                        this.item.items[ind].value = '';
                        break;
                    default:
                        this.item.items[ind].value = '0';
                }
