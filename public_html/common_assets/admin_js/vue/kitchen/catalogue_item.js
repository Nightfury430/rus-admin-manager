...
document.addEventListener('Glob_ready', async function () {
    ...
    requests.push(promise_request(glob.base_url + '/catalog/categories_get/catalogue'));
    requests.push(promise_request(glob.base_url + '/catalog/items_get/catalogue'))
    ...
    handles_hash = get_hash(handles_data.items);
    handles_cat_hash = get_hash(handles_data.categories);
...
function init_vue() {
    app = new Vue({
        ...
        mounted: function(){
            ...
            let inter = setInterval(function () {
                ...
                if(scene !== undefined){
                    ...
                    set_obj.materials['__no_mat'] = {
                        "name": "Материал удален",
                        "type": "m2",
                        "default": "v_16",
                        "key": "__no_mat",
                        "variants": {
                    ...
                    if(item != null){
                        ...
                        for (let i = scope.item.category.length; i--;){
                            if(!cat_hash[scope.item.category[i]]){
                                scope.item.category.splice(i ,1)
                            }
                        }
        ...
        created: function(){
            ...
            this.$options.handles_data = handles_data;
            this.$options.handles_hash = handles_hash;
            this.$options.handles_cat_hash = handles_cat_hash;
        ...
        methods: {
            ...
            change_fill_size(axis){
                this.$selected_object.userData.params.spec.fill.size[axis] = this.item.params.spec.fill.size[axis];
            },
            ...
            get_rest_handles: function(){
                let scope = this;
                let handles = this.$selected_object.userData.params.spec.rest.handles;
                if(handles != null){
                    this.rest_handles_mode = handles.mode;
                    switch (handles.mode) {
                        case 'i':
                            for (let i = handles.items.length; i--;){
                                if(!this.$options.handles_hash[handles.items[i]]){
                                    handles.items.splice(i, 1);
                                }
                            }

                            if(handles.items.length > 0){
                                this.use_handles_rest = 1;
                                this.rest_handles = copy_object(handles.items);
                                this.rest_handles_default = copy_object(handles.default);
                            }
                        ...
                        default:
                            ...
                            for (let i = handles.items.length; i--;){
                                if(!this.$options.handles_cat_hash[handles.items[i]]){
                                    handles.items.splice(i, 1);
                                }
                            }
            ...
            change_rest_handles(){
                let obj = {
                    "mode": this.rest_handles_mode,
                    "default": this.rest_handles_default,
                    "items": copy_object(this.rest_handles)
                }

                this.$selected_object.userData.params.spec.rest.handles = obj
            },

            get_cats: function(){
                return copy_object(this.rest_handles)
            },

            change_rest_handles_mode(){
                this.rest_handles = [];
                this.rest_handles_default = 0;
            ...
            change_rest_handles_on(){
                if(this.use_handles_rest == 0){
                    this.$selected_object.userData.params.spec.rest.handles = null
                } else {
                    this.change_rest_handles();
                }
            },

            change_rest_handles_default(e){
                this.rest_handles_default = e;
                this.change_rest_handles();
            },
            ...
            submit: async function (e) {
                ...
                let url = glob.base_url + '/catalogue/item_add';
