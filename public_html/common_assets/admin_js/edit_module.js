...
$(document).ready(function () {
    ...
    if(!project_settings.project_id) project_settings.project_id = new Date().getTime();
    ...
    cab_params = JSON.parse($('#item_params').val());

    setTimeout(function () {

        for(var i = 0; i < cab_params.params.variants.length; i++){
            if(cab_params.params.variants[i].default === "1"){
                cab_params.params.variants[i].default = true;
            }
        }

        cab = new Cabinet_new(JSON.parse(JSON.stringify(cab_params.params)));
        scene.add(cab);
        cab.box.update();
        cab.position.y = -cab.box.getSize().y / 2;
