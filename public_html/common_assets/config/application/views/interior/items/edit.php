...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10">
        <h2><?php echo $lang_arr['interior_item_edit']?></h2>
        <!--        <ol class="breadcrumb">-->
...
<form class="add_item" enctype="multipart/form-data" method="post" accept-charset="UTF-8" action="<?php echo site_url('interior/items_edit/'.$item['id']) ?>">
                                ...
                                <div class="panel panel-default panel-body sizes_row row">
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control alias_input" value="<?php if(isset($variant->alias)) echo $variant->alias; ?>">
                                        </div>
                                    </div>
	                                ...
	                                <?php if($key != 0 ):?>
                                        <div class="col-sm-12">
                                            <div class="form-group">
	                                            <?php if (!empty($variant->model)):?>
                                                    <label class="cur_model" data-model="<?php echo $variant->model?>"><?php echo $lang_arr['current_model']?> <?php echo $variant->model?></label>
	                                            <?php endif;?>
                                                <label><?php echo $lang_arr['choose_file']?></label>
                                                <input type="file" name="variant_model_<?php echo $key?>" class="form-control model_input">
                                            </div>
                                        </div>
	                                <?php endif;?>
                ...
                <div class="ibox-content">
                    ...
                    <div class="form-group row">
                        <div class="col-sm-4 col-sm-offset-2">
                            <a class="btn btn-white btn-sm" href="<?php echo site_url('interior/items_index/'.$_SESSION['selected_interior_items_category'].'/'.$_SESSION['selected_interior_items_per_page'].'/'.$_SESSION['selected_interior_items_pagination']) ?>"><?php echo $lang_arr['cancel']?></a>
...
<script>
    ...
    function add_size_row(){
        ...
        var alias_col = $('<div class="col-sm-12"></div>');
        var alias_fg = $('<div class="form-group"></div>');
        var alias_label = $('<label><?php echo $lang_arr['name']?> </label>');
        var alias_input = $('<input type="text" class="form-control alias_input" value="">');

        var model_col = $('<div class="col-sm-12"></div>');
        var model_fg = $('<div class="form-group"></div>');
        var model_label = $('<label><?php echo $lang_arr['choose_file']?> </label>');
        var model_input = $('<input type="file" name="variant_model_'+ ($('.sizes_row').length) +'" class="form-control model_input" value="">');
        ...
        alias_col.append(alias_fg);
        alias_fg.append(alias_label);
        alias_fg.append(alias_input);
        row.append(alias_col);

        model_col.append(model_fg);
        model_fg.append(model_label);
        model_fg.append(model_input);
    ...
    $(document).ready(function () {
        ...
        $('.add_item').submit(function (e) {
            ...
            $('.sizes_row').each(function () {
                ...
                console.log(scope.find('.model_input')[0])
                ...
                if(i > 1){
                    if( parseFloat(scope.find('.width_input').val()) > 0 && parseFloat(scope.find('.height_input').val()) > 0 && parseFloat(scope.find('.depth_input').val()) > 0)
                        if(scope.find('.model_input')[0].files[0]){
                            select.append('<option selected value="'+ scope.find('.width_input').val() + ';' + scope.find('.height_input').val() + ';' + scope.find('.depth_input').val() + ';' + scope.find('.alias_input').val() +';' + scope.find('.model_input')[0].files[0].name +'"></option>')
                        } else {

                            if(scope.find('.cur_model')){
                                select.append('<option selected value="'+ scope.find('.width_input').val() + ';' + scope.find('.height_input').val() + ';' + scope.find('.depth_input').val() + ';' + scope.find('.alias_input').val() + ';' + 'curmodel_' + scope.find('.cur_model').attr('data-model') +'"></option>')
                            } else {
                                select.append('<option selected value="'+ scope.find('.width_input').val() + ';' + scope.find('.height_input').val() + ';' + scope.find('.depth_input').val() + ';' + scope.find('.alias_input').val() +'"></option>')
                            }
