    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-10">
            <?php if(isset($common) && $common == 1):?>
                <?php if(isset($id)):?>
                    <h2 style="color: red">{{lang(controller_name)}} ({{lang('edit')}})<span> (ОБЩАЯ БАЗА)</span> <span v-cloak>{{item.name}}</span></h2>
                <?php else:?>
                    <h2 style="color: red">{{lang(controller_name)}} ({{lang('add')}}) (ОБЩАЯ БАЗА)</h2>
                <?php endif;?>
            <?php else:?>
                <?php if (isset($id) && $id != 0):?>
                    <h2>{{lang(controller_name)}} ({{lang('edit')}}) <span v-cloak>{{item.name}}</span></h2>
                <?php else:?>
                    <h2>{{lang(controller_name)}} ({{lang('add')}})</h2>
                <?php endif;?>
            <?php endif;?>
        </div>
        <div class="col-lg-2">
                                ...
                                <div class="row">
                                    <div class="col-6">
                                        <div class="form-group">
                                            <div>{{lang('current_model')}}</div>
                                            <pp_model :lang="$options.lang" ref="test_model" :model="item.model" v-model="item.model"></pp_model>
                                        </div>
                                        <div class="form-group row">
                                            <label class="col-sm-6 col-form-label">{{lang('width')}}, {{lang('units')}}</label>
                                            <div class="col-sm-6">
                                                <input type="number" v-model="item.params.width" min="0" step="1" class="form-control">
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            <label class="col-sm-6 col-form-label">{{lang('height')}}, {{lang('units')}}</label>
                                            <div class="col-sm-6">
                                                <input type="number" v-model="item.params.height" min="0" step="1" class="form-control">
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            <label class="col-sm-6 col-form-label">{{lang('depth')}}, {{lang('units')}}</label>
                                            <div class="col-sm-6">
                                                <input type="number" v-model="item.params.depth" min="0" step="1" class="form-control">
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            <label class="col-sm-6 col-form-label">{{lang('smes_rotation')}}, {{lang('degrees')}}</label>
                                            <div class="col-sm-6">
                                                <input type="number" v-model="item.params.smes_rotation" min="0" step="1" class="form-control">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        ...
                                        <template v-for="(it,ind) in item.items">
                                            <tr>
                                                <td class="col-6">
                                                    <div v-if="ind != 0">
                                                        <div class="form-group">
                                                            <select @change="change_item_price_type(ind)"  v-model="it.type" class="form-control">
                                                                <option value="0">{{lang('category')}}</option>
                                                                <option value="1">{{lang('mat_code')}}</option>
                                                                <option value="2">{{lang('id_short')}}</option>
                                                            </select>
                                                        </div>

                                                        <div v-if="it.type == 0" class="form-group">
                                                        ...
                                                        <div v-if="it.type == 1" class="form-group">
                                                            <label>{{lang('mat_code')}}</label>
                                                            <input v-model="it.value" type="text" class="form-control form-control-sm">
                                                        </div>
                                                        <div v-if="it.type == 2">
                                                            <pp_items v-if="options_ready" :categories="[]"  @e_update="it.value=$event" :unselect="true" :selected_item="it.value" :lang="$options.lang" :controller="'materials'"></pp_items>
                                                        </div>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="col-6">
                                                    <div>
                                                        <pp_items v-if="options_ready"  @e_update="it.acc_id=$event" :unselect="true" :selected_item="it.acc_id" :lang="$options.lang" :controller="'accessories'"></pp_items>
                                                    </div>
                                                    <div v-show="it.acc_id == 0" class="text-center">{{lang('or')}}</div>
                                                    <table v-show="it.acc_id == 0" class="table table-bordered table-sm mb-0">
                                                        <thead>
                                                        <tr>
                                                            <th>{{lang('name')}}</th>
                                                            <th>{{lang('code')}}</th>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <input v-model="it.price" type="number" class="form-control form-control-sm">
                                                            </td>
                                                        </tr>
                                                        </tbody>
...
<script src="/common_assets/admin_js/vue/kitchen/washes_item.js?<?php echo md5(date('m-d-Y-His A e')); ?>"></script>
