...
//            $gres_categories = array_non_empty_items($this->glass_model->get_active_categories());
	        ...
	        if($constructor_settings['shop_mode'] == 1){
		        ...
		        foreach ($kitchen_models as $item){
			        ...
			        $it->materials->walls = json_decode($ps['available_materials_walls']);
			        $it->materials->floor = json_decode($ps['available_materials_floor']);
			        ...
			        if($constructor_settings['multiple_facades_mode'] == 1){
				        ...
				        $it->prices_data = json_decode($item['price_data']);
...
//                                    $fac_data_mod_vars->price = intval($fac_data_mod_vars->price) * $constructor_settings['price_modificator'];
            ...
            if($constructor_settings['shop_mode'] == 1 ) {
                ...
                foreach ($module_sets as $set) {
                    ...
                    foreach ($set_modules as $s_mod) {
                        ...
                        $m->params = json_decode($s_mod['params'])->params;
...
//            $comms_categories = $this->comms_model->get_active_categories();
...
//            $interior_categories = $this->interior_model->get_active_categories();
...
//            $handles_categories = $this->handles_model->get_active_categories();
            ...
            if($constructor_settings['shop_mode'] == 1){
                ...
                $project_settings->materials->top->corpus = json_decode($ps['available_materials_corpus']);

                $project_settings->materials->bottom = new stdClass();
                $project_settings->materials->bottom->facades = $c_mats;
                $project_settings->materials->bottom->corpus = json_decode($ps['available_materials_corpus']);

                $project_settings->materials->cokol = json_decode($ps['available_materials_cokol']);

                if(isset($ps['available_materials_glass'])){
                    $project_settings->materials->glass = json_decode($ps['available_materials_glass']);
                }



                $project_settings->materials->tabletop = json_decode($ps['available_materials_tabletop']);
                $project_settings->materials->walls = json_decode($ps['available_materials_walls']);
                $project_settings->materials->floor = json_decode($ps['available_materials_floor']);
                $project_settings->materials->wall_panel = json_decode($ps['available_materials_wallpanel']);
...
//                print_r($project_settings);
                ...
                $project_settings->selected_materials->wall_panel = (int)$ps['selected_material_wallpanel'];
                ...
                $project_settings->hinges = new stdClass();
                $project_settings->hinges->id = 0;
                $project_settings->hinges->name = 'С доводчиком';
                $project_settings->hinges->icon = '/common_assets/images/with_dovod.jpg';
                $project_settings->hinges->door_hinges_price = 1000;
                $project_settings->hinges->locker_hinges_price = 1000;
                $project_settings->hinges->dovodchik = true;
                ...
                $project_settings->available_corpus_thickness = json_decode($ps['available_corpus_thickness']);
...
//
//            $tabletop_plints->categories = $this->common_model->get_data_all_by_order('Tabletop_plints_categories', 'ASC');
//            $tabletop_plints->items = $this->common_model->get_data_all_by_order('Tabletop_plints_items', 'ASC');
            ...
            if( !$this->materials_model->check_mat_active($project_settings->selected_materials->top->facades)  ){
                ...
                if($constructor_settings['shop_mode'] == 1){
                    ...
                    $errors[] = 'Не выбран материал фасадов в разделе "Предустановленные декоры" или выбранный материал не активен';
                }
            ...
            if( !$this->materials_model->check_mat_active($project_settings->selected_materials->top->corpus)  ){
                ...
                if($constructor_settings['shop_mode'] == 1){
                    ...
                    $errors[] = 'Не выбран материал корпуса в разделе "Предустановленные декоры" или выбранный материал не активен';
            ...
            if( !$this->materials_model->check_mat_active($project_settings->selected_materials->cokol)  ){
                ...
                if($constructor_settings['shop_mode'] == 1){
                    ...
                    $errors[] = 'Не выбран материал цоколя в разделе "Предустановленные декоры" или выбранный материал не активен';
            ...
            if( !$this->materials_model->check_mat_active($project_settings->selected_materials->tabletop)  ){
                ...
                if($constructor_settings['shop_mode'] == 1){
                    ...
                    $errors[] = 'Не выбран материал столешницы в разделе "Предустановленные декоры" или выбранный материал не активен';
                }

            }

            if( !$this->materials_model->check_mat_active($project_settings->selected_materials->wall_panel)  ){

                if($constructor_settings['shop_mode'] == 1){
                    $errors[] = 'Не выбран материал фартука в модели кухни или выбранный материал не активен';
                } else {
                    $errors[] = 'Не выбран материал фартука в разделе "Предустановленные декоры" или выбранный материал не активен';
                }
            ...
            if( !$this->materials_model->check_mat_active($project_settings->selected_materials->walls)  ){
                ...
                if($constructor_settings['shop_mode'] == 1){
                    $errors[] = 'Не выбран материал стен в разделе "Предустановленные декоры" или выбранный материал не активен';
                } else {
                    $errors[] = 'Не выбран материал стен в разделе "Предустановленные декоры" или выбранный материал не активен';
                }
            ...
            if( !$this->materials_model->check_mat_active($project_settings->selected_materials->floor)  ){
                if($constructor_settings['shop_mode'] == 1){
                    $errors[] = 'Не выбран материал пола в разделе "Предустановленные декоры" или выбранный материал не активен';
                ...
                if($this->config->item('sub_account') != true){
                    ...
                    if(basename(dirname(FCPATH)) == 'antarescompany2.ru'){
                        ...
                        foreach ($module_sets_catalog as $id=>$set){
                            file_put_contents(dirname(FCPATH).'/data/module_sets/' . $id . '.json', json_encode($set));
                        }
                        ...
                        write_file(dirname(FCPATH).'/data/kitchen_models.json', json_encode($kitchen_models_catalog));
...
//                    write_file(dirname(FCPATH).'/data/tabletop_plints.json', json_encode($tabletop_plints));
...
//                                        $db_to_path = dirname(dirname(FCPATH)) . '/' . $sub_acc.'/data/db';
//                                        $db = new SQLite3($db_to_path);
//
//                                        $all_mods = $this->module_sets_model->get_all_items();
//                                        $all_cats = $this->module_sets_model->get_all_categories();
//
//                                        foreach ($all_mods as $s_mod){
//                                            $q = "UPDATE Module_sets_items SET category=:category, icon=:icon, params=:params, template_id=:template_id, 'order'=:order, set_id=:set_id WHERE id=:id";
//                                            $statement = $db->prepare($q);
//                                            foreach ($s_mod as $kf=>$field){
//                                                if($kf != 'active'){
//                                                    $statement->bindValue(':'. $kf .'', $field);
//                                                }
//                                            }
...
//                                            $q = "UPDATE Module_sets_categories SET name=:name, parent=:parent, active=:active, description=:description, 'order'=:order, set_id=:set_id WHERE id=:id";
//                                            $statement = $db->prepare($q);
//                                            foreach ($s_mod as $kf=>$field){
//                                                $statement->bindValue(':'. $kf .'', $field);
//                                            }
...
//
//
//                                        $query = $db->query("SELECT * FROM Module_sets_items");
//
//                                        $hash = [];
//
//
...
//
//
//                                        foreach ($sub_kitchen_models_result as $ke=>$ss){
//                                            foreach ($ss->available_modules->items as $ko=>$mod){
//
//
//
//
//
//                                                if(isset($hash[$mod->id])){
//                                                    if($hash[$mod->id] == 0){
//                                                        array_splice($sub_kitchen_models_result[$ke]->available_modules->items, $ko, 1);
//                                                    }
//                                                }
//
//
