...
class Interior extends CI_Controller {
	...
	private $controller_name = "interior";
...
//		$data['categories'] = buildTree($this->interior_model->get_categories(), 'parent', 'id');
		...
		$data['header'] = $data['lang_arr']['interior_categories_list'];
...
//		$this->load->view('common/categories_index', $data);
	...
	public function categories_add()
	{
		...
		if ($this->form_validation->run() === FALSE)
		{
			...
			$this->load->view('templates/header', $data);
			$this->load->view('interior/categories/add', $data);
			$this->load->view('templates/footer', $data);
		}
		else
		{
			$this->interior_model->add_category();
			redirect('interior/categories_index/', 'refresh');
		}
	}
	...
	public function categories_edit($id)
	{
		...
		if ($this->form_validation->run() === FALSE)
		{
			$data['category'] = $this->interior_model->get_category($id);
			$data['has_children'] = $this->interior_model->check_category_for_children($id);
			...
			$this->load->view('templates/header', $data);
			$this->load->view('interior/categories/edit', $data);
			$this->load->view('templates/footer', $data);
		}
		else
		{
			$this->interior_model->update_category($id);
			redirect('interior/categories_index/', 'refresh');
		}
	}
	...
	{
		if($this->interior_model->check_category_for_children($id) == 0){
			$this->interior_model->remove_category($id);
		}
	...
	public function categories_set_active_ajax($id, $val) {
		$this->interior_model->set_category_active($id, $val);
	}
    ...
    {
        if($this->input->post()) $this->interior_model->categories_order_update(json_decode($this->input->post('data'), true));
    }

    //todo
    public function categories_get()
    {
        echo json_encode($this->interior_model->get_categories());
    }
    ...
    {
        if($this->input->post()){
            echo $this->interior_model->add_category_ajax(
                array(
    ...
    {
        if($this->input->post()){
            echo $this->interior_model->update_category_ajax(
    ...
    {
        if($this->input->post()) $this->interior_model->remove_category((int)$this->input->post('id'));
    }
	...
	public function items_index($category = false, $per_page = false, $start = false)
	{
		...
		if($category == null || $per_page == null){
			...
			redirect('interior/items_index/'.$category .'/'.$per_page.'/', 'refresh');
		}

		$_SESSION['selected_interior_items_category'] = $category;
		$_SESSION['selected_interior_items_per_page'] = $per_page;
		$_SESSION['selected_interior_items_pagination'] = $start;


		$this->load->library('pagination');
		$config['base_url'] = site_url('interior/items_index/'.$category .'/'.$per_page.'/');
		$config['total_rows'] = $this->interior_model->get_items_count($category);
		...
		$data['items'] = $this->interior_model->get_items($category, $start, $config['per_page']);
		$data['categories'] = $this->interior_model->get_categories();
		$data['materials_categories'] = buildTree($this->interior_model->get_categories(), 'parent', 'id');
		...
		$this->load->view('templates/header', $data);
		$this->load->view('interior/items/index', $data);
		$this->load->view('templates/footer', $data);
	}
	...
	public function items_remove($id)
	{
		...
		if (is_dir(dirname(FCPATH).'/models/interior/'. $id)) {
			delete_files(dirname(FCPATH) . '/models/interior/' . $id, true);
			rmdir(dirname(FCPATH) . '/models/interior/' . $id);
		}
		$this->interior_model->remove_items($id);
		redirect('interior/items_index/'.$_SESSION['selected_interior_items_category'].'/'.$_SESSION['selected_interior_items_per_page'], 'refresh');
	}
    ...
    public function items_add_ajax_common($id = false) {
        if($this->input->post()){
            ...
            if(($id)){
                ...
                $this->common_model->add_data( 'Interior_items', $model_data);
            }
	...
	{
		$data['items'] = $this->interior_model->get_catalog_items();
		$data['categories'] = $this->interior_model->get_catalog_categories();

		$data['acc_categories'] = $this->interior_model->get_categories();
		$this->load->model('languages_model');
		$data['lang_arr'] = get_default_lang();
		if($this->config->item('ini')['language']['language'] !== 'default'){
			$custom_lang = json_decode($this->languages_model->get($this->config->item('ini')['language']['language']));
		...
		$this->load->view('templates/header', $data);
		$this->load->view('interior/items/catalog', $data);
		$this->load->view('templates/footer', $data);
	}
	...
	{
		$cat_item = $this->interior_model->get_catalog_item($id);


		$data = array();

		$item_id = $this->interior_model->add_item();
		...
		echo json_encode($this->interior_model->add_item_data($item_id, $data));
	}

	public function set_active_ajax($id, $val) {
		$this->interior_model->set_active($id, $val);
	}
	...
	public function item_get_ajax( $id ) {
		echo json_encode($this->interior_model->get_item($id));
	}
	...
	public function items_add_ajax($id = false) {
		if(isset($_POST)){
			...
			$this->interior_model->add_item_data($item_id, $model_data);
		}
	}
    ...
    public function copy_to_catalog_common($id=0, $category = 1)
    {
        ...
        $item = $this->interior_model->get_item($id);
        ...
        $item_id = $id;
//        $common_id = 24;
        $common_id = $this->common_model->add_data('Interior_items', array('name'=>''));


        $models_path = dirname(FCPATH).'/models/interior/'. $item_id;
        $common_assets_path = $_SERVER['DOCUMENT_ROOT'] . '/common_assets/models/interior/' . $common_id;
...
//        print_r($this->common_model->get_items_count('Facades_items', 'Facades_categories', 0));
        ...
        $old_model_string = 'models/interior/' . $item_id;
        $new_model_string = '/common_assets/models/interior/' . $common_id;
...
//        print_pre($common_item);
        ...
        $this->common_model->update_where('Interior_items', 'id', $common_id, $common_item);
        $this->interior_model->add_item_data($id, $item);
