...
class Modules_sets_modules extends CI_Controller {
...
//    }
    ...
    public function preview_set($set_id)
    {
        ...
        $this->load->view('templates/header', $data);
        $this->load->view('module_sets/modules_sets_modules_preview', $data);
        $this->load->view('templates/footer', $data);
...
//			}
    ...
    public function modules_settings() {
        ...
        $this->load->view('templates/header', $data);
        $this->load->view('modules/settings', $data);
        $this->load->view('templates/footer', $data);
    }
...
//            continue;
    ...
    public function get_item_data_ajax($id) {
        $this->load->model('common_model');
        echo json_encode($this->common_model->get_row_where_common('Modules_sets_modules_items', array('id' => $id)));

//        echo json_encode($this->modules_model->get_item($id));
    }

    public function get_data_ajax($set_id = false) {
...
//		$data['categories'] = $this->modules_model->get_categories_by_parent($top_category);
//        $data['categories'] = $this->modules_model->get_categories();
        $data['categories'] = $this->common_model->get_data_all_by_order_module_set_temp('Modules_sets_modules_categories', $set_id, 'ASC');
//		$data['modules_templates'] = $this->modules_templates_model->get_items_by_category_common($top_category);
...
//		$data['modules_templates_custom'] = $this->modules_templates_model->get_items_by_category($top_category);
    ...
    public function get_preview_data_ajax($set_id = false) {
...
//        $data['categories'] = $this->modules_model->get_categories();
        $data['categories'] = $this->common_model->get_data_all_by_order_module_set_temp('Modules_sets_modules_categories', $set_id, 'ASC');
        $data['items'] = $this->common_model->get_data_all_by_order_module_set_temp('Modules_sets_modules_items', $set_id, 'ASC');
        $f_data = $this->common_model->get_row_where_common('Modules_sets_items', array('id' => $set_id));
        $data['facade_set'] = $this->common_model->get_row_where_common('Facades_systems_items', array('id' => $f_data['facade_set_id']));
        echo json_encode($data);
    }

    public function make_icon()
    {
        if(isset($_POST['icon'])){
            $img = $_POST['icon'];
            ...
            if(!file_exists($_SERVER['DOCUMENT_ROOT']. '/common_assets/models/modules_icons/' . $_POST['set_id'])){
                mkdir($_SERVER['DOCUMENT_ROOT']. '/common_assets/models/modules_icons/' . $_POST['set_id']);
            }

            $file = $_SERVER['DOCUMENT_ROOT']. '/common_assets/models/modules_icons/' . $_POST['set_id']. '/'.  $_POST['id'] .'.' . 'jpg';
            $success = file_put_contents($file, $data);
            $path = '/common_assets/models/modules_icons/' . $_POST['set_id']. '/'.  $_POST['id'] .'.' . 'jpg';
            $this->load->model('common_model');
            $this->common_model->update_where_common('Modules_sets_modules_items', 'id', $_POST['id'], array(
                'icon'=> $path
            ));
    ...
    public function export_modules($set_id)
    {
        $this->load->model('common_model');
        $modules_categories = $this->common_model->get_data_all_by_order_module_set_temp('Modules_sets_modules_categories', $set_id, 'ASC');
        $modules_items = $this->common_model->get_data_all_by_order_module_set_temp('Modules_sets_modules_items', $set_id, 'ASC');

        $modules  = array();

        $modules_catalog = array();
        ...
        foreach ($modules_items as $item){
            ...
            $it = array(
                ...
                'icon' => $item['icon'],
                'params' => json_decode($item['params'], true)['params']
            );
        ...
        $path ='/common_assets/fs/common/modules_' . $set_id .'.json';
        file_put_contents( $_SERVER['DOCUMENT_ROOT'] . $path, json_encode($modules));
        echo json_encode($path);
    }


    public function get_facade_set_by_set_id_common($set_id)
    {
        $this->load->model('common_model');

        $data = $this->common_model->get_row_where_common('Modules_sets_items', array('id' => $set_id));

        echo json_encode($this->common_model->get_row_where_common('Facades_systems_items', array('id' => $data['facade_set_id'])));
    }
