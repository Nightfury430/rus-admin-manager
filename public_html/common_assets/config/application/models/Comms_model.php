...
class Comms_model extends CI_Model {
    ...
    public function get_categories()
    {
        ...
        $query = $this->db->get('Comms_categories');
        return $query->result_array();
    }

    public function get_category($id)
    {
        $query = $this->db->get_where('Comms_categories', array('id' => $id));
        return $query->row_array();
    }

    public function get_active_categories()
    {
        $this->db->from('Comms_categories');
    ...
    public function add_category()
    {
        ...
        return $this->db->insert('Comms_categories', $data);
    }

    //todo copy
    public function add_category_ajax($data)
    {
        $this->db->insert('Comms_categories', $data);
    ...
    {
        $this->db->update_batch('Comms_categories', $data, 'id');
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('Comms_categories', $data);
        return $id;
    }
    ...
    public function update_category($id)
    {
        ...
        if(count($children) > 0){
            foreach ($children as $child){
                ...
                $this->db->update('Comms_categories', array('active' => $this->input->post('active')));
            }
        }
    }
    ...
    public function remove_category($id)
    {
        ...
        $delete = $this->db->delete('Comms_categories');
    ...
    {
        $this->db->limit(1);
        $query = $this->db->get_where('Comms_categories', array('parent' => $id));
	...
	public function set_category_active( $id, $val ) {
		...
		if($cat['parent']){
			...
			if($parent['active']){
				$this->db->where('id', $id);
				$this->db->update('Comms_categories', array('active'=>$val));
    ...
    public function get_items($category, $offset, $limit)
    {
        if($category == 0){
            $this->db->order_by('order','ASC');
            $query = $this->db->get('Comms_items',$limit,$offset);
            return $query->result_array();
        } else {

            $children = $this->get_category_children($category);

            $this->db->from('Comms_items');
    ...
    {
        $query = $this->db->get_where('Comms_items', array('id' => $id));
        return $query->row_array();
    }
    ...
    {
        $this->db->insert('Comms_items', array('name'=>''));
        return $this->db->insert_id();
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('Comms_items', $data);
    }
    ...
    public function remove_items($id)
    {
        ...
        $delete = $this->db->delete('Comms_items');
        return $delete?true:false;
    }
    ...
    {
        if($category == 0){
            return $this->db->count_all('Comms_items');
        } else {
            $children = $this->get_category_children($category);
            $this->db->from("Comms_items");
            $this->db->where('category', $category);
	...
	public function set_active( $id, $val ) {
		$this->db->where('id', $id);
		$this->db->update('Comms_items', array('active'=>$val));
	}
    ...
    public function get_catalog_items()
    {
        ...
        $query = $this->db_common->get('Comms_items');
        return $query->result_array();
    }

    public function get_catalog_item($id)
    {
        $query = $this->db_common->get_where('Comms_items', array('id' => $id));
        return $query->row_array();
    }
    ...
    {
//        $this->db_common->order_by('id','DESC');
        $query = $this->db_common->get('Comms_categories');
        return $query->result_array();
    }
