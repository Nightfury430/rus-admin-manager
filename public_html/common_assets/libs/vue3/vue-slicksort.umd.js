(function (global, factory) {
    ...
    function resetTransform(nodes = []) {
        for (let i = 0, len = nodes.length; i < len; i++) {
            ...
            el.classList.remove("pp-tree-t", "pp-tree-b", "pp-tree-c");
        }
    }
    ...
    const ContainerMixin = vue.defineComponent({
        ...
        mounted() {
            ...
            this.container = this.$el.closest(".pp-tree-container");
                ...
                const onEnd = () => {
                    ...
                    if (this.hub && !this.hub.isDest(this)) {
                        ...
                        this.$emit('sort-end', {
                            event: e,
                            oldIndex: this.index,
                            newIndex: this.bottom ? this.newIndex + 1 : this.newIndex,
                            inside: this.inside && !this.bottom,
                        });
            ...
            animateNodes() {
                ...
                for (let i = 0, len = nodes.length; i < len; i++) {
                    ...
                    if (this._axis.y) {
                        if (index === this.index
                            && sortingOffset.top + scrollDifference.top + offset.height >= edgeOffset.top
                            && sortingOffset.top + scrollDifference.top + offset.height <= edgeOffset.top + offset.height * 2) {

                            this.newIndex = index;

                            const a = offset.height * 2 / 6;
                            const s = sortingOffset.top + scrollDifference.top + offset.height;

                            borderTop = (s >= edgeOffset.top) && (s <= edgeOffset.top + a);
                            borderBottom = (s >= edgeOffset.top + offset.height * 2 - a) && (s <= edgeOffset.top + offset.height * 2);
                            inside = (s > edgeOffset.top + a) && (s < edgeOffset.top + offset.height * 2 - a);
                            this.inside = inside;
                            this.bottom = borderBottom;
                        }
                        else if (index > this.index && sortingOffset.top + scrollDifference.top + offset.height >= edgeOffset.top) {
                    ...
                    node.classList[borderTop ? "add" : "remove"]("pp-tree-t");
                    node.classList[borderBottom ? "add" : "remove"]("pp-tree-b");
                    node.classList[inside ? "add" : "remove"]("pp-tree-c");
